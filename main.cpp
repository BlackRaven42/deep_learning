...
int main(int argc, char* argv[]) {
    ...
    for (i = 1; i < argc; ++i) {
        if (!strcmp(argv[i], "-h")) {
            ...
            std::cout << "-tr - number of samples using for traing (int)" << std::endl;
            std::cout << "-ts - number of samples using for checking (int)" << std::endl;
            return 0;
        }
        if ((!strcmp(argv[i], "-hu")) && (i + 1 < argc)) {
            ...
            train_set_num = atoi(argv[i + 1]);
            ++i;
        }
        else if ((!strcmp(argv[i], "-ts")) && (i + 1 < argc)) {
            test_set_num = atoi(argv[i + 1]);
            ++i;
        }
    }
    ...
    train_set_num = std::min(train_set_num, (int)train_images.size());
    ...
    test_set_num = std::min(test_set_num, (int)test_images.size());
